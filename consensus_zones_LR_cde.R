...
#pitches <- pitches[1:500,] # for testing
...
for(s in c("L", "R")) {
  ...
  stkKDE[[s]] <- kde2d(stk[[s]]$px, stk[[s]]$pz, n=50, lims = c(-2,2,0,5))
  cpKDE[[s]] <- kde2d(cp[[s]]$px, cp[[s]]$pz, n=50, lims = c(-2,2,0,5))
  czKDE[[s]] <- stkKDE[[s]]
...
saveRDS(szcontourdf, "upper90kde17cde.Rda")
